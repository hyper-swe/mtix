...
**Conflict baseline.** Every export and every applied import records a hash of the local store, the conflict baseline (`.mtix/data/sync-db.sha256`). When the file changed and the store no longer matches that hash, both changed since the last sync: nothing is imported, and the refusal is recorded as a `conflict` (see the table below). **After upgrading from 0.5.3 or earlier**, the baseline on disk is the one the older version recorded, over its 1.0.0 export, which lacked the fields 0.5.4 exports (comments, activity and the other fields added in 2.0.0). A baseline can also predate the uids a store gives its tasks without one when it opens. mtix recognizes such a baseline: when the store, unchanged, matches it in that older form, mtix records the baseline again in the current form and logs `sync_baseline_upgraded` once, and the first pull after the upgrade imports as usual, whether or not other commands ran before it. A change you made before upgrading and never exported is still a conflict. The exception is a change only to a field the 1.0.0 export lacked, which 0.5.3 did not detect either; the loss check below still compares it. If mtix reports a conflict although you changed nothing, `mtix import .mtix/tasks.json --mode merge` combines both sides. Do not use `mtix sync --fix` for this: it drops every change in the file.
