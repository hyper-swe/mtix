...
### Upgrading
- **Upgrade every teammate who shares a `.mtix/tasks.json` board to 0.5.4 or later, and until then keep older clients from writing (MTIX-95.31.1).** 0.5.4 writes the board as `schema_version` 2.0.0, which carries annotations, the activity stream and the other node fields. A client older than 0.5.4 supports only 1.x, so its automatic import skips the file and logs `tasks.json schema version is newer than supported — upgrade mtix`; its `mtix import` of the file fails with `checksum verification failed`, because it drops the fields it does not know. Nearly every node has activity entries, so this applies to practically every board 0.5.4 writes, not only to boards with comments. Skipping the file protects the older client's store only until its next writing command: every write re-exports `.mtix/tasks.json` from that store, which lacks everything it skipped and every annotation, and records the file as in sync. Committing that file reverts every change made upstream since the client stopped importing. So, until every teammate runs 0.5.4 or later, do not run writing commands on an older client and do not commit its `.mtix/tasks.json`. If such a file was committed, restore `.mtix/tasks.json` from the last good commit in git history, or restore the store of a machine that imported it from `.mtix/data/pre-sync-backup.db` (the copy taken before the last automatic import; copy it over `.mtix/data/mtix.db` while no mtix process runs). A 0.5.4 store is protected from such a file by the auto-import guard (MTIX-95.31.2), which refuses a replace import that would drop annotations or nodes the store holds. 0.5.4 reads boards written by older clients (1.x). `.mtix/tasks.json` grows, because it now carries every comment and each node's activity.
