...
## 16. Addendum B — merge import of a board another clone renumbered (2026-09-25, MTIX-95.31.4)

§6 (offline / export-import) rejects an incoming `uid` that the local store holds under a *different* `display_path`, and applies a live-store mutation only with confirmation. That wrongly strands a clone once the git-tracked board (`.mtix/tasks.json`) is shared by several clones: when clone A's merge renumbers its own task `PROJ-3` to `PROJ-4` (because B's board gives `PROJ-3` to a different task), every clone that already held A's task at `PROJ-3` then sees the same `uid` at `PROJ-4` in the board, and its import was rejected with a uid conflict.

**Exception (merge imports only):** an incoming `uid` that the local store holds under a different `display_path` **under the same parent** is the same node, renumbered by another clone. The merge **moves** the local node, with its subtree, to the file's `display_path` before it looks for different tasks under the same id. The move is **reported and printed** (`local tasks moved to the id the file holds them under`, and in the `--remap-file` output) but **needs no confirmation**: the node keeps its `uid`, follows the published board, nothing is overwritten, and no two nodes end up sharing a `uid`. The move runs inside the import transaction through the §5 renumber path. A `uid` held under a *different* parent is still rejected (or re-stamped with `--force-rename`), and replace imports are unchanged.

**Related rules of the same change:** a local node that holds an id the file gives to a *different* task (another `uid`) is never overwritten: the merge renumbers the local node to the next number free in both the store and the file, and applies that only **with** confirmation, as §6 requires. Two nodes under one id with different `uid`s are the same task only when their `created_at` values are equal and at least one `uid` was assigned after the task was created: a **marked backfill uid** (a UUIDv8 whose 12 bits after the version hold `0xBF1`), whatever its time, which mtix gives a node that reached the store without a `uid` when it is imported or when the store opens, and which the §7 local-mint backfill now mints too (MTIX-95.31.9); or a UUIDv7 whose embedded time is **more than one hour after** `created_at`, assigned by the §7 local-mint backfill of an earlier release when a clone upgraded from before uids were shared. Every other pair is two different tasks: a UUIDv7 time before `created_at` (a node a hub applied records the apply time as `created_at`), within the hour after it (a create that waited for the write lock), or a `uid` that is neither a UUIDv7 nor a marked backfill uid. When either node has no `uid` to compare (a board written before uids were shared), the titles decide: another title is a different task (MTIX-95.31.9). The rule is deliberately asymmetric: a wrong "different" costs a renumber the user confirms, a wrong "same" loses a task. A create mints its `uid` right after it reads the clock, before any step that waits for the write lock.
