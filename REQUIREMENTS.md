...
**FR-15.8** (Concurrent access) Auto-import and auto-export MUST acquire an advisory file lock on `.mtix/data/sync.lock` before reading or writing `.mtix/tasks.json`. The lock MUST use `flock(2)` semantics (shared lock for read, exclusive lock for write). If the lock cannot be acquired within 5 seconds, the operation MUST be skipped with a warning. This prevents corruption when multiple mtix processes run concurrently (e.g., parallel CI jobs, multiple terminal sessions).
