...
### Added
- **`mtix sync repair --status` heals workflow state that an older pull reverted (MTIX-95.6).** Before 0.5.4, a pull re-applied events this client had already pushed (MTIX-95.2) and applied claims and status changes in arrival order (MTIX-95.10), so a task could be left in an older state: claim, push, `mtix done`, pull left it `in_progress` with `closed_at` still set. Upgrading stops new damage but does not change such tasks. `mtix sync repair --status` re-derives each task's workflow state from its newest well-formed claim, unclaim, defer or status-change event in the local event log, with the rule a pull now applies, and lists every task whose status, assignee, agent_state, `closed_at` (set or not) or, for a task without children, progress differs, where that event writes the field; for an event this client made, `closed_at` follows this client's own history (its last `done` or cancel sets it, a reopen clears it). Each listed task shows its winning event, when and on which machine it was made, and a reason. A *replay* (the stored state is what an older event of the task writes, which is what a replayed pull left, shown as the replayed event) and a *derived fix* (the status is right, only `closed_at` or progress is not) are repaired by `--apply`. Any other difference is *flagged "not a replay; review"*: the stored state is what no older event writes, or the task's activity records a status change after the winning event, or the task is cancelled under a cancelled ancestor. Such state can be newer than the event log, for example after importing a teammate's `.mtix/tasks.json`, so `--apply` skips it and only `--apply --force` repairs it, after review. A clock difference between machines can flag a genuine replay. It is a dry run by default, and `--json` prints the list. `--apply` first writes a verified backup of the database to `.mtix/data/backups/pre-repair-status-<UTC time>.db` (with a `-2` suffix and so on when the name is taken) and stops without changing anything if it cannot. It then repairs each task in its own transaction: it writes the derived state, adds an activity entry naming the winning event, recomputes the parent's progress and re-exports `.mtix/tasks.json`. Only a repair that changes the status emits an event: one status-change event with the reason `sync repair`, stamped with the winning event's time so other clients keep the same `closed_at`, which they apply at their next pull and which fires `status.changed` hooks; it also unblocks dependents. A repair of `closed_at`, progress, the assignee or agent_state alone emits nothing. Run `mtix sync push` afterwards; a second run lists nothing. A done task without children gets progress 1.0, which a `done` received by sync never set before 0.5.4. Left alone: tasks without such events; a blocked task with an unresolved blocker and a task cancelled by `mtix cancel --cascade` on an ancestor, which are not synced as events; an assignee set later with `mtix update --assignee`; the wake time of this client's own deferral; `closed_at` after this client invalidated or restored the task; and the exact `closed_at` time, of which only whether it is set is compared. The command reads and writes only the local database and never contacts the hub.
