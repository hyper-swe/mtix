...
**Related rules of the same change:** a local node that holds an id the file gives to a *different* task (another `uid`) is never overwritten: the merge renumbers the local node to the next number free in both the store and the file, and applies that only **with** confirmation, as §6 requires. Two nodes under one id with different `uid`s are the same task only when their `created_at` values are equal and at least one `uid` was not minted at creation (a UUIDv7 whose embedded time lies within [`created_at` − 1 s, `created_at` + 2 s] was minted at creation; any other `uid` was assigned later by a backfill, §7). Two tasks two clones created in the same second both carry creation-minted `uid`s and stay different tasks.
