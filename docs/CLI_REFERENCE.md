...
> Auto-generated by mtix 0.5.3-beta. Do not edit manually.
...
- `hooks` — Inspect and test FR-19 event hooks (.mtix/hooks.yaml)
- `import <file>` — Import nodes from JSON export
- `inbox` — Show events addressed to an agent
...
- `projects` — List projects in the database with node counts
...
- `unblock <id>` — Re-derive a blocked node's status from its blockers; clear it if all are resolved
...
| Flag | Short | Description | Default |
|------|-------|-------------|---------|
| `--all-projects` |  | Span all projects | false |
...
| Flag | Short | Description | Default |
|------|-------|-------------|---------|
| `--to` |  | Address the comment to an agent (delivers to its inbox) |  |
---
...
Run the origin-independent dispatch loop: every interval (default 5s),
...
- `install` — Register the daemon as an OS service (boot-start, crash-restart)
- `start` — Start the installed daemon service
- `status` — Show the installed daemon service's state
- `stop` — Stop the installed daemon service
- `uninstall` — Unregister the daemon service and remove its unit file
---
...
Register the daemon as an OS service (boot-start, crash-restart)

### Flags

| Flag | Short | Description | Default |
|------|-------|-------------|---------|
| `--interval` |  | Pull-then-dispatch interval in seconds baked into the service | 5 |
---
...
Show the installed daemon service's state
---
...
Unregister the daemon service and remove its unit file
---
...
Inspect and test FR-19 event hooks (.mtix/hooks.yaml)

### Subcommands

- `exec-dispatch [any|daemon|off]` — Get or set this host's local exec dispatch policy
...
**Usage:** `exec-dispatch [any|daemon|off]`

Get or set this host's local exec dispatch policy

Get (no argument) or set which trigger on THIS host may run exec hooks:
...
Test hooks.yaml against a sample event (dry-run only, for now)

### Flags

| Flag | Short | Description | Default |
|------|-------|-------------|---------|
| `--dry-run` |  | preview matches without delivering (currently the only supported mode) | false |
| `--event` |  | event name (required): comment.addressed, status.changed, node.created |  |
| `--from` |  | origin/author agent (drives the from-agent-not filter) |  |
| `--node` |  | affected node id (drives the `under` subtree filter) |  |
| `--status-to` |  | new status (drives the status-to filter) |  |
| `--synced` |  | treat the event as arriving via hub replication | false |
| `--to` |  | addressee agent (drives the to-agent filter) |  |
---
...
List configured hooks with their events and delivery adapters
---
...
Show recent hook firings (audit trail, newest first)
...
Trust the current .mtix/hooks.yaml to run exec hooks (content-hash pinned, local)

Record the current .mtix/hooks.yaml as trusted to run its exec hooks.
...
| Flag | Short | Description | Default |
|------|-------|-------------|---------|
| `--status` |  | Show whether the current hooks.yaml is trusted, without changing it | false |
---
...
Show events addressed to an agent

Show comments addressed to an agent (via 'mtix comment --to <agent>'),
...
| Flag | Short | Description | Default |
|------|-------|-------------|---------|
| `--agent` |  | Agent id whose inbox to read (required) |  |
| `--format` |  | Agent-ready output: 'prompt' (wake payload) or 'context' (hook injection) |  |
| `--timeout` |  | Max seconds to block with --wait | 300 |
| `--wait` |  | Long-poll: block until an event arrives or --timeout | false |

### Subcommands

- `ack <seq>...` — Acknowledge specific inbox events (selective); --through acks everything up to a seq
---
...
Acknowledge specific inbox events (selective); --through acks everything up to a seq

Acknowledge inbox events. By default this is SELECTIVE: only the
...
|------|-------|-------------|---------|
| `--agent` |  | Agent id (required) |  |
| `--through` |  | Acknowledge every event up to and including the given seq (watermark), not just that seq | false |
---
...
| Flag | Short | Description | Default |
|------|-------|-------------|---------|
| `--channel-agent` |  | Also act as a Claude Code channel (research preview) for this agent id: push its new inbox events into the running session (FR-20 §9 rung 4). Requires the session to be launched with --channels (or the development flag while the preview allowlist applies) |  |
| `--project` |  | Project directory containing .mtix/ (overrides cwd) |  |
| `--read-only` |  | Serve only read/query tools; refuse mutations (MTIX-2.1.3). Overrides the mcp.read_only config key. Use for untrusted or observer MCP clients. | false |
---
...
List projects in the database with node counts
---
...
DEPRECATED: use 'mtix daemon' — the first-class dispatcher loop
...
| `--yes` |  | Execute instead of previewing; does NOT bypass the typed confirmation --discard-local requires | false |
---
...
- `attach <dir>` — Join an existing relay
- `clone` — Export or import a bootstrap snapshot
- `init <dir>` — Create a relay in a shared directory
- `republish` — Re-emit this peer's events from an earlier relay sequence
- `reset-peer` — Declare a new publisher epoch after restoring this store from backup
- `retire-peer <peer-id>` — Drop a vanished peer from the prune quorum
- `rotate-key` — Install the next key epoch and record its boundary
- `status` — Show relay peers, positions and outstanding problems
- `tick` — Run exactly one relay pass: publish, then ingest
---
...
Export or import a bootstrap snapshot

A peer joining after pruning cannot tail from the start. Any current
...
Create a relay in a shared directory

### Flags

| Flag | Short | Description | Default |
|------|-------|-------------|---------|
| `--no-auth` |  | Create an unauthenticated relay (§8.3 opt-out; records carry no MAC) | false |
---
...
Re-emit this peer's events from an earlier relay sequence

Operator gap repair. Re-emits into FRESH segments — no existing file
...
| Flag | Short | Description | Default |
|------|-------|-------------|---------|
| `--from` |  | Relay sequence to repair from | 0 |
---
...
Declare a new publisher epoch after restoring this store from backup

Use after a restore, when publishing has refused with
...
| Flag | Short | Description | Default |
|------|-------|-------------|---------|
| `--base-rs` |  | Relay sequence the new epoch starts at | 1 |
| `--floor` |  | Journal position to republish from | 0 |
---

## retire-peer

**Usage:** `retire-peer <peer-id>`

Drop a vanished peer from the prune quorum

A peer that never acks holds every writer's retention window open
...
Install the next key epoch and record its boundary
---
...
Show relay peers, positions and outstanding problems
...
Run exactly one relay pass: publish, then ingest

One pass and exit — for peers that cannot host a daemon:
...
Re-derive a blocked node's status from its blockers; clear it if all are resolved

Re-derive a node's blocked status from its current blockers.
