...
- The merge lists every uid it adopts, applied or not: `uids adopted from the file (the same task under another uid): N`, then one line per task, `PROJ-3 local uid=<uid> -> file uid=<uid> (local "<your title>", file "<their title>")`. `--json` carries them as `uid_adoptions` (`id`, `local_uid`, `file_uid`, `local_title`, `file_title`), and `--remap-file` maps each local uid to its id, so a reference to the old uid still resolves. Check every adoption whose titles differ. One case gets past the rule: two different tasks created in the same second on clones running releases before 0.4, when both clones upgraded more than an hour later, both carry uids assigned at upgrade, so mtix treats them as one task and the merge keeps the file's task under the id. The report then adds `a task above whose titles differ may be two different tasks ...`, and the backup the merge took first (its path is printed before the report) holds your task. To recover it, stop every mtix process (the MCP server included), copy that backup over `.mtix/data/mtix.db` to return to the state before the merge, copy your task to a new one (`mtix show PROJ-3`, then `mtix create` with its title and description), and merge again. The automatic import lists such a pair, when the titles differ, as `PROJ-3: treated as the same task (uid assigned at upgrade) (local "<your title>", file "<their title>")` and refuses, even when nothing else would be lost; its merge option names each such task. If the titles name two different tasks, do not merge yet: copy your task to a new one first, as above, then merge. The opposite case is safe but can look wrong: when both clones upgraded less than an hour after a task was created, its two uids count as minted at creation, so the two copies of that one task are listed as `a different task under this id` with identical titles, and the merge renumbers your copy only with `--confirm`. If both copies are the same task, merge with `--confirm`, check that the renumbered copy holds nothing the other lacks (annotations, activity), and delete it with `mtix delete <new id>`.
