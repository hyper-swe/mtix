...
- **Annotations and activity in merge mode (MTIX-95.31.1):** a merge MUST NOT drop a local annotation or activity entry. Annotations merge as a union by annotation id, so an incoming node without annotations keeps the local ones; for an id both sides hold, the local copy wins unless the incoming copy is resolved and the local one is not (a resolution never regresses). Activity entries merge as a union. A file whose `schema_version` is older than 1.1.0 carries none of the node fields 1.1.0 added, so a merge MUST keep their local values.
...
- **Time validation (MTIX-95.31.1):** before writing anything, import MUST reject a time value that is not RFC 3339 or whose UTC year is outside 1..9999 (node timestamps, annotation and activity times, dependency, agent and session times), with an error naming the record and the field.
...
**FR-15.1** mtix MUST maintain a git-tracked export file at `.mtix/tasks.json` containing the full task hierarchy in the same format as `mtix export`. "Full" means every column of every node, including annotations (in the structure `mtix show --json` returns) and the activity stream, covered by the export checksum (`schema_version` 1.1.0, MTIX-95.31.1). A nodes column MAY be left out only if it is derived, local-only or a cache, and each one left out MUST be listed with its reason in `docs/EXPORT-FORMAT.md`; today none is. A file written by mtix MUST verify when read back, including a copy restored by git (MTIX-107.39).
