...
- **Synced machines:** Agents on different machines can each claim the same node between syncs. After `mtix sync pull`, the most recent claim or status change (highest Lamport clock, ties broken by event id) holds on every machine and older ones change nothing; the losing claim is not reported as a conflict. After every pull, check the assignee with `mcp__mtix__mtix_show`; if another agent now holds your node, stop work on it and pick other work
