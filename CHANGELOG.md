...
### Added
- **`mtix sync repair --status` heals workflow state that an older pull reverted (MTIX-95.6).** Before 0.5.4, a pull re-applied events this client had already pushed (MTIX-95.2) and applied claims and status changes in arrival order (MTIX-95.10), so a task could be left in an older state: claim, push, `mtix done`, pull left it `in_progress` with `closed_at` still set. Upgrading stops new damage but does not change such tasks. `mtix sync repair --status` re-derives each task's workflow state from its newest well-formed claim, unclaim, defer or status-change event in the local event log, with the rule a pull now applies, and lists every task whose status, assignee, agent_state, `closed_at` (set or not) or, for a task without children, progress differs, where that event writes the field; for an event this client made, `closed_at` follows this client's own history (its last `done` or cancel sets it, a reopen clears it). Each listed task shows its winning event, when it was made, whether this machine or another machine made it, and a reason. A *replay* (the stored state is what an older event of the task writes, which is what a replayed pull left, shown as the replayed event) and a *derived fix* (the status is right, only `closed_at` or progress is not) are repaired by `--apply`. Any other difference is *flagged "not a replay; review"*: the stored state is what no older event writes, or the task's activity records a status change after the winning event, or only the assignee or agent_state differs (a later `mtix update --assignee` leaves no trace), or the task is cancelled and an ancestor was cancelled after its winning event (possibly a cascade cancel). Such state can be newer than the event log, for example after importing a teammate's `.mtix/tasks.json`, so `--apply` skips it and only `--apply --force` repairs it, after review. Clocks that differ between machines can mislead the check both ways: a genuine replay can be flagged, and when this machine's clock is ahead, a teammate's newer state can be listed as a replay that `--apply` would revert, so check each replay's winner time and origin before `--apply`. It is a dry run by default, and `--json` prints the list. Run `mtix sync pull` before listing and again just before `--apply` (MTIX-95.35): a repair event carries this machine's newest Lamport clock, so on a log that lacks a teammate's newer, unpulled change it can win over that change (whenever this machine's Lamport clock is ahead of it) and revert it on every machine; when it lists differences, the dry run ends with a reminder to pull first (the `reminder` field in `--json`, omitted when there is no reminder). `--apply` first writes a verified backup of the database to `.mtix/data/backups/pre-repair-status-<UTC time>.db` (with a `-2` suffix and so on when the name is taken) and stops without changing anything if it cannot. It then repairs each task in its own transaction: it writes the derived state, adds an activity entry naming the winning event, recomputes the parent's progress and re-exports `.mtix/tasks.json`. Only a repair that changes the status emits an event: one status-change event with the reason `sync repair`, stamped with the winning event's time so other clients keep the same `closed_at` (or with the current time when that time is in the future, since `mtix sync push` refuses an event more than a day ahead), which they apply at their next pull and which fires `status.changed` hooks; it also unblocks dependents. A repair of `closed_at` or progress alone (or, with `--force`, of the assignee or agent_state) emits nothing. Run `mtix sync push` afterwards; a second run lists nothing. A done task without children gets progress 1.0, which a `done` received by sync never set before 0.5.4. Left alone: tasks without such events; a blocked task with an unresolved blocker and a task cancelled by `mtix cancel --cascade` on an ancestor without a cancel event of its own, which are not synced as events; an assignee set later with `mtix update --assignee` (flagged when it arrives by import); the wake time of this client's own deferral; `closed_at` after this client invalidated or restored the task; and the exact `closed_at` time, of which only whether it is set is compared. The command reads and writes only the local database and never contacts the hub.

### Fixed
- **The automatic import after a `git pull` deleted local data the pulled `.mtix/tasks.json` lacked, silently (MTIX-95.31.2).** When a `git pull` or checkout changed `.mtix/tasks.json`, the next mtix command, even `mtix show`, replaced the local store with the file, so annotations, activity entries, tasks, dependencies and field values (an assignee, for example) that only the local store held were deleted without a word; a teammate's board without the local annotations took them from 2 to 0. The automatic import now compares the node and dependency data of the store with the file first (agents and sessions are runtime state and are still replaced). A task, an annotation, an annotation's resolution, an activity entry or a dependency the file lacks is always a loss (so a teammate's deliberate `mtix dep remove` is refused too), and so is a local task whose id the file gives to a different task (both carry a uid and the uids differ, as when two clones each create `PROJ-3`, even in the same second), listed as `a different task under this id` with both titles. A task the file holds under another id with the same uid (another clone renumbered it) is not a loss, and two copies under different uids are one task only when they share a creation time and one uid (a UUIDv7, which carries the time it was minted) was assigned more than an hour after that time, by a clone upgraded from before uids were shared; the file's uid is then adopted, and an empty uid never replaces one. A task mtix imports without a uid is now given one in the import (a backfill uid: a UUIDv8 with a fixed marker, which the identity rule treats as assigned after the task was created), and a store that holds tasks without a uid gives them one when it opens, logging how many, so every board 0.5.4 writes carries a uid for every task, and two clones that each backfilled the same task keep one task (MTIX-95.31.9, MTIX-95.31.10). That backfill on open runs through the write pre-flight and fail-stop like any write, a merge gives a local task without a uid one before it decides identity, and a board without uids no longer counts a missing uid as a loss for a task whose local uid is a backfill uid: the replace keeps it, so a teammate's boards from a client at 0.3 or older keep importing. When the file's copy has no uid to compare (a board from a client at 0.3 or older, or one not re-exported since), the titles decide: another title is a different task, which the automatic import lists as `a task under this id with a different title and no uid to compare` with both titles, and which `mtix import --mode merge` renumbers with `--confirm` instead of overwriting (before, it replaced the local title and description without a word), while the same title is merged as before (MTIX-95.31.9). `mtix import` lists every uid it adopts, with the id, both uids and both titles (on stderr; `uid_adoptions` in `--json` once the import is applied; `--remap-file` maps the local uid to the id), because the rule has one known residual: two different tasks created in the same second, when at least one clone assigned its uid after the task was created (at upgrade, or as a backfill uid on import or open, for example for a task created before 0.2), are treated as one task, the merge keeps the file's under the id, and the backup taken before the merge holds the local one. When such a pair's titles differ, the automatic import refuses and lists it as `treated as the same task (uid assigned at upgrade)` with both titles, and its merge option says the merge takes the file's title and content for it and to copy your task with `mtix create` first, so you can refuse the merge (MTIX-95.31.6). The opposite case is safe: two copies of one task whose uids were all assigned less than an hour after it was created are listed as `a different task under this id`, with identical titles, and renumbered only with `--confirm`. Every other pair is two tasks, including two created in the same second or a create that waited for the write lock; a create now mints its uid before it waits. A field value the file leaves empty is a loss unless the file's copy of the task is current: it holds all of the task's local activity entries and its `updated_at` is not older, so a stale copy, one from before a local `mtix update` or `mtix delete` (which record no activity), or one from a client older than 0.5.4 (which writes no activity) cannot blank a value, while a field a teammate cleared on purpose (unclaim, reopen, undefer, undelete, a later update) applies. The rule fails open in one direction: a copy with a later `updated_at` that missed a local `mtix update` or `mtix delete`, after a concurrent edit to the same task or with a teammate's clock ahead, counts as current, so a field it leaves empty is applied as cleared; the pre-import backup keeps the state before. On a loss the import is refused: nothing is imported, no backup is taken and the stored hash is kept, so the refusal repeats, printed once, on every command. It lists what would be lost, task by task, and three ways to proceed, run from the project root, each of which resolves it: `mtix import .mtix/tasks.json --mode merge` (backs up the database first, unless the merge would change nothing, and keeps everything the refusal lists: every local task, annotation, activity entry and dependency, and every listed field value that the file's stale copy leaves empty, with the task's local status when the value is part of it; it adds the file's changes, and for a task whose content is unchanged the local field values win, so it can undo a teammate's field change), `mtix sync --fix` (keeps the local store and rewrites the file from it) and `mtix import .mtix/tasks.json --mode replace` (the file wins). This protects a 0.5.4 store from a board re-exported by an older client: it is always refused. A board that only adds or changes data is imported as before, now with one line on stderr: `mtix: imported the changed .mtix/tasks.json: nodes N added, N updated, N removed; dependencies N added, N removed (local database backed up to ...)`. A successful automatic import now also refreshes the conflict baseline, so a second pull before any write is imported instead of being reported as a conflict. A conflict (both the store and the file changed since the last sync) whose replace would also lose data is now printed as a refusal with the loss list, which `mtix sync` keeps showing, and its replace option names what the replace deletes. The replace re-checks, inside its own transaction, that the store is unchanged since the comparison: a write that lands in between, from another process or the MCP server, is kept, nothing is imported, and the next command checks the file again. `mtix import --mode merge` no longer overwrites a local task with a different task under the same id (before, it replaced the task's title, description and uid, and no backup held the old one): it renumbers the local task and its subtree to the next number free in both the store and the file, keeping the uid, annotations and activity, and imports the file's task at the id, so the published board keeps its numbers; the renumbering is printed (uid, old id, new id; `--remap-file` saves it) and applied only with `--confirm`, and without it nothing is written. A clone that holds a task another clone renumbered moves it to the board's id, without `--confirm`. `mtix sync` no longer prints `In sync` while an automatic import of the board is pending or an id carries another uid in the file. `mtix import --mode replace` itself is not guarded.
...
- **`mtix sync pull` could permanently miss changes a teammate pushed after working offline (MTIX-95.5).** Pull asks the hub for the events whose Lamport clock is above this client's cursor, and that clock is stamped by the client that made the change. A teammate who worked offline pushes events stamped below the cursors of teammates who kept working, so those teammates never pulled them and their copies silently diverged. After its cursor pass, pull now sweeps for late events: it lists the ids of the hub events created since its previous sweep, minus a 15-minute overlap, and stages the ones this client does not hold; once the listing is complete it applies all the staged events through the same path, in Lamport order across the whole sweep, reading them from the staging table `--limit` at a time so memory and each transaction stay bounded. A late push can also straddle a teammate's cursor (a task's creation stamped below it and an edit above it), which made the regular fetch fail on the edit and every later pull fail the same way; now such an edit is quarantined (MTIX-95.11), the sweep brings the creation, and the same pull then applies the edit, so a task's creation applies before its edits in both cases. The window and the recorded sweep time come from the hub's clock only, so a wrong clock on the client has no effect. A recovered claim, unclaim, defer or status change that is older than the task's newest one is recorded as received and changes nothing (the MTIX-95.10 rule). The first pull after upgrading compares the full hub event history once, in pages, and prints `late-event sweep (first run, full hub history): N late events recovered`, so changes missed before the upgrade arrive then; later pulls print `late-event sweep: N late events recovered` only when they recover something. A sweep saves its listing position with each page's staged ids, and staged events stay staged until applied, so on a large hub a comparison that a pull cannot finish in time (a daemon pull stops after 60 seconds) continues on the next pull instead of starting over. In the common case (nothing missing, and at most `--limit` events since the last sweep) the sweep adds one hub query to a pull; a larger window adds a listing query per further `--limit` ids, and recovered events a fetch per `--limit` of them. It runs only during a pull and adds no timer, so an idle hub that scales to zero stays idle. Hub migration 014 adds an index on `sync_events (created_at)`, created only when absent, so the sweep reads only recent events; migrations need the hub owner, so an existing hub gets the index the next time its owner runs `mtix sync init`, and until then each page the sweep lists scans the hub's event table (one scan per pull for the usual window; the one-time full comparison takes one per page, spread across pulls). `mtix sync status` shows the last sweep time (`last sweep`; `last_sweep_at` in `--json`) and whether the first full comparison is still in progress (`full_sweep_in_progress`, with `sweep_pending_events` counting staged events, in `--json`); `mtix sync clone` and `mtix sync reconcile --discard-local` clear the sweep time, the saved progress and the staged events, so the next pull compares the full history again. The staging table (`sync_sweep_pending`) is created automatically in the local database; the schema version does not change. A recovered event that cannot be applied is quarantined like one from the cursor pass (MTIX-95.11), and the pull succeeds. Known limit: the overlap covers push transactions of normal length, which take seconds; an event from a stalled push transaction that started more than 15 minutes before a sweep and committed after it is not recovered by later pulls.
- **`mtix sync pull` and `mtix sync clone` could permanently skip events that share a Lamport clock, and the first pull after a clone read the whole hub again (MTIX-95.4).** Both asked the hub for the events above the highest Lamport clock of the previous batch. Lamport clocks are not unique, so the events at that clock that did not fit in the batch were never pulled or cloned, and that replica's copy silently diverged. Pull and clone now page by `(lamport_clock, event_id)`: the pull cursor keeps the event id of the last event pulled next to its clock (`meta.sync.last_pulled_event_id`, and `meta.sync.clone.checkpoint_event_id` for `mtix sync clone --resume`), and each batch starts after the last event received, so every event is fetched once however `--limit` or `--batch-size` splits the log. Pull saves its cursor in the transaction that applies the batch, quarantined events included, so a crash can no longer leave applied events behind an older cursor; the daemon's pull does the same. Clone did not set the pull cursor, although its documentation said so, and the first pull after it fetched the whole hub log again; clone now sets it when it completes, so that pull fetches only newer events. The new keys are added empty the first time 0.5.4 opens a store (the schema version does not change); the first pull after upgrading therefore fetches the events at its saved clock again, skips the ones it has and applies any it had missed. `mtix sync reconcile --discard-local` resets both parts of the cursor. Hub migration 015 adds an index on `sync_events (lamport_clock, event_id)`, created only when absent, so each pull batch reads only its own rows; migrations need the hub owner, so an existing hub gets the index the next time its owner runs `mtix sync init`. `mtix sync init` runs all migrations in one transaction that locks the hub's event table until it commits, so pushes and pulls both wait for it, longer on a large hub while the index builds (a waiting push or pull retries for about 50 seconds and normally completes once init finishes; if the wait outlasts its retries it fails, nothing is lost, and the next one succeeds): run it when a pause in sync is acceptable. Until then pulls fetch the same events, only slower.
...
- **`mtix sync pull` applied hub events without the push-side checks, and one event it could not apply stalled every later pull (MTIX-95.11).** Pull checked only the format of a pulled event. An event over the FR-18.7 caps (payload size or depth, Lamport or vector-clock limits, id grammars) was applied, and an extreme Lamport clock was adopted, after which the hub refused every event this client made. Any event whose apply failed, such as a dependency whose target had not arrived yet, rolled back its whole batch, and a hub row whose vector clock did not decode failed the whole pull, so the pull failed the same way every time. Pull now checks each event before applying it: first its Lamport clock, which must be below 2^53 and at most `sync.max_lamport_jump` above the local clock (a new config key, default 4294967296, 2^32; `mtix config set sync.max_lamport_jump <n>` takes a positive integer), then the FR-18.7 caps. An event stamped more than 24 hours ahead of this machine's clock is applied with a warning. Each event applies in its own savepoint: an event that fails a check or its apply, or whose hub row does not decode, is rolled back and kept in a new local table, `sync_quarantine`, with the reason, and the pull goes on past it. A quarantined event never moves the local clock, and the pull cursor never moves to a refused clock, whatever else is wrong with the event. Only an event already recorded as applied skips the checks; the hub copy of an event this client pushed is checked like any other until a pull acknowledges it, and acknowledging it merges the clocks of the local copy, not of the hub copy, so a hub row changed after the push cannot move the local clocks. Every pull retries the quarantine first, before contacting the hub, and again after it applied events, so a dependency or edit whose target arrives later applies then, and a row whose event the store has already applied is dropped; this replaces the one retry of the cursor pass that MTIX-95.5 added for an edit met before its node's create. The new read-only command `mtix sync quarantine list` (`--json`) lists quarantined events with their reasons, `mtix sync status` shows `quarantined events` (`quarantined_events` in `--json`), and `mtix sync doctor` has a `quarantined events` check that fails while any remain. `mtix sync reconcile --discard-local` empties the quarantine. `mtix sync clone` still applies all or nothing and has no quarantine: it now runs the same checks on every hub event before it writes anything, and refuses the whole clone, naming the event and the reason, when any fails; on the fresh store, `mtix sync pull` quarantines such an event instead. The table is created on the next start of mtix, with no schema version change.
- **One event the hub would refuse stopped every later `mtix sync push` (MTIX-95.12).** The hub accepts an event payload of at most 64 KB and refuses a whole push batch that holds an invalid event, but local fields may be larger: a new task's prompt may be 100 KB and its description 50 KB, and acceptance criteria, comments and later edits of a description or prompt have no local size limit. One such change failed its batch on every push, and because the queue head never moved, that client stopped pushing altogether. Push now checks each pending event with the hub's rules before it sends a batch. An event the hub would refuse is held: it stays in the local `sync_events` table as pending and is recorded in `sync_quarantine` with source `push` and a reason that starts with its kind. A payload over the limit (`too large:`, naming the largest field) or another broken rule (`refused:`) is held for good. An event stamped more than 24 hours ahead of the machine's clock (`temporary: clock:`) is checked again at the start of every push and released once it passes. 0.5.x events name a task by its number only, so while a task's creation is held, none of the changes of its subtree are sent: push holds every event of that task and of the tasks below it, including changes made after the last push. It finds the task an event is about by the task's internal id, not its number, so a change made before the task was renumbered (by an import merge on this machine, even one another process commits while the push runs, or by a renumber the hub asks for during a push; push decides each batch with its held creations and the batch's tasks read at the same database version, and reads them again and decides the batch again when another process committed in between) is still held, and a change of a task that later takes one of its old numbers is not. The same holds when the task's internal id changed after its creation was queued, because a merge import gave the task the id the board holds for it or the creation was queued before changes carried one: push finds the task by the creation's own id or else by the number the creation names, and holds its later changes and child tasks too (a task that has taken that number since is held as well; known limit: if a task whose id a merge import changed is then renumbered on this machine, its changes that no push checked before that renumber are not recognized). If `mtix gc` purges a deleted task of the subtree, its held changes stay held and changes of a task whose own creation is held are still found by uid; others are checked by the number they name (known limit: a change under a number from a local renumber that no push checked before the purge is not recognized). A dependency link or unlink names its target by number only, and no record of a task's earlier numbers is kept, so while any creation is held, every link or unlink made after it is held too (its reason says links made while a task creation is held wait for it), whatever task it names, until no creation made before it is held. The reason names the nearest held creation (`depends on held create of <event id> (<task>)`). Once a clock hold on a creation clears, the creation and the changes of its subtree go through the ordinary push in the same run, where a renumber by the hub is a known limit of this version (MTIX-95.37): if a teammate took the number while the creation was held, the hub renumbers the creation and the changes sent with it keep the old number. Before it releases them, push checks each change of the subtree with the hub's rules; one the hub would refuse stays held under its own reason, keeping its first-held time and attempt count, and a child creation among them keeps its own subtree held. A creation held for good keeps its whole subtree held. The rest of the batch is pushed, later pushes leave held events out, and a held event is never deleted, or sent while it is held; changes of other tasks keep pushing, except the links described above. Push names each newly held or released event on stderr and prints `held: N events not pushed` at the end. `mtix sync status` shows `held push events` (`held_push_events` in `--json`; held events are also still counted in `pending`), `mtix sync doctor` has a new `held push events` check that fails while any is held and names the fix for each event (shorten or split a field edit; escalate a held task creation, which has no automatic re-send; a dependent resolves with the held creation it depends on; a held link pushes once no creation made before it is held; check the clock for a clock hold), its `queue draining` check no longer counts held events, and `mtix sync quarantine list` lists them (`"source": "push"` in `--json`). Pull never retries, removes or counts them as quarantined pulled events, and `mtix sync clone --resume` keeps them. With a hub configured, a CLI command or MCP tool call whose change makes a sync event over the limit still succeeds, and prints one warning line naming the field and the limit (on stderr; for MCP as an extra text block of the result); for a task's creation it says the creation will be held with every later change of the task and must not be edited to fix it. Changes made through the web UI, the REST API or gRPC are not warned about, though push still holds their events; imports write no sync events, so nothing they write is pushed or held. A `sync_quarantine` table created by an earlier 0.5.4 development build is widened to accept source `push` on the next start, keeping its rows; there is no schema version change and no protocol change.
...
### Upgrading
- **Hub owner: run `mtix sync init` once after upgrading, when a pause in sync is acceptable (MTIX-95.4).** It adds the hub indexes that 0.5.4 pulls read by, `idx_sync_events_created_at` (migration 014, the late-event sweep) and `idx_sync_events_lamport_event_id` (migration 015, the pull cursor). All migrations run in one transaction that locks the hub's event table until it commits, so pushes and pulls both wait for it, longer on a large hub while the indexes build. `mtix sync init` stops after 30 seconds; if the build takes longer it changes nothing. Pulls are correct without the indexes, only slower.
