...
### Upgrading
- **Hub owner: run `mtix sync init` once after upgrading, when a pause in pushes is acceptable (MTIX-95.4).** It adds the hub indexes that 0.5.4 pulls read by, `idx_sync_events_created_at` (migration 014, the late-event sweep) and `idx_sync_events_lamport_event_id` (migration 015, the pull cursor). All migrations run in one transaction, so while the indexes build pushes wait and pulls keep working; on a large hub this can take a while. Pulls are correct without the indexes, only slower.
