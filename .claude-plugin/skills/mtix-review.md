...
Read the comments and review verdicts already on a node with `mtix show <id>`: it prints every annotation, oldest first, with its author and timestamp, and marks resolved ones (`--json` gives the complete record). `mtix context` includes only unresolved annotations.
