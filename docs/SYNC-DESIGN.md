...
- **Workflow events** (`claim`, `unclaim`, `transition_status`, `defer`). One workflow state per node, not per field. At apply time an event wins only if its (`lamport_clock`, `event_id`) is higher than that of every well-formed workflow event the replica already holds for the node (a Lamport tie goes to the higher `event_id`); a loser is recorded as applied but changes nothing and logs no conflict. A malformed workflow event (a `transition_status` with no usable to-status, or a `transition_status`, `claim` or `defer` whose payload cannot be decoded) never counts as held and changes nothing; see the malformed workflow events item of the residual list in [SYNC-PROTOCOL.md](SYNC-PROTOCOL.md#workflow-events-last-writer-wins-at-ingest) (MTIX-95.27). A winner writes the columns its local mutation writes, and a terminal `closed_at` comes from the winner's `wall_clock_ts`. Status converges in any arrival order for claims and status changes that travel as events, and `closed_at` converges among replicas that received the events by sync; local writes that emit no event (auto-block, cascade-cancelled descendants), the originator exceptions and the secondary-column limits are listed there. The rule, the column table and the 0.5.x residual are specified in [SYNC-PROTOCOL.md](SYNC-PROTOCOL.md#workflow-events-last-writer-wins-at-ingest) (MTIX-95.10).
