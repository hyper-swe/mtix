...
### Added
- **`mtix sync repair --status` heals workflow state that an older pull reverted (MTIX-95.6).** Before 0.5.4, a pull re-applied events this client had already pushed (MTIX-95.2) and applied claims and status changes in arrival order (MTIX-95.10), so a task could be left in an older state: claim, push, `mtix done`, pull left it `in_progress` with `closed_at` still set. Upgrading stops new damage but does not change such tasks. `mtix sync repair --status` re-derives each task's workflow state from its newest well-formed claim, unclaim, defer or status-change event in the local event log, with the rule a pull now applies, and lists every task whose status, assignee, agent_state, `closed_at` (set or not) or, for a task without children, progress differs, where that event writes the field. It is a dry run by default, and `--json` prints the list. `--apply` first writes a verified backup of the database to `.mtix/data/backups/pre-repair-status-<UTC time>.db` and stops without changing anything if it cannot. It then repairs each task in its own transaction: it writes the derived state, adds a `status_change` activity entry, emits one status-change event with the reason `sync repair` that other clients apply at their next pull, recomputes the parent's progress, unblocks dependents and re-exports `.mtix/tasks.json`; run `mtix sync push` afterwards. A second run lists nothing. A done task without children also gets progress 1.0, which a `done` received by sync never set before 0.5.4. Left alone: tasks without such events; a block added by a new dependency and a descendant cancelled by `mtix cancel --cascade`, which are not synced as events; an assignee set later with `mtix update --assignee`; the wake time of this client's own deferral; and the exact `closed_at` time, of which only whether it is set is compared. The command reads and writes only the local database and never contacts the hub. Known limits: a task whose state came from importing `.mtix/tasks.json` rather than from events is listed as different, so review the list before `--apply`; and on other clients a repaired `done` or `cancelled` task takes the time of the repair as its `closed_at`, because the repair is a new status change.
