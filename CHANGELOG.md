...
### Fixed
- **After `mtix sync clone` or `mtix sync pull`, `mtix create` could fail with "already exists" (MTIX-95.38).** Applying a cloned or pulled task did not advance the local ID counter of its parent, so the next `mtix create` under that parent took a number a pulled task already held and failed with `node <ID> already exists`. Each failed create moved the counter on by one, so after a clone of k tasks, or a pull of k new tasks numbered above the counter, k creates failed before one succeeded. A renumber (after the hub rejected a task's number, or when a provisional ID settles) did not update the counters either, so a create under a renumbered task could take the number of one of its moved subtasks. Applying a cloned or pulled task now advances its parent's counter in the same transaction (a re-applied event leaves it unchanged), a renumber raises the counter of every parent it touches, and a create whose counter is behind for any other reason, for example an import that stopped before it rebuilt the counters, takes the number after the highest one under the parent instead of failing. No counter is ever lowered.
