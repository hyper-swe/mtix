...
- `checksum` — SHA-256 hex digest of the canonical JSON encoding of the `nodes` and `dependencies` arrays (computed before embedding into the outer envelope, arrays sorted by primary key). The `agents` and `sessions` arrays and the envelope fields are not covered; whether to cover them is tracked as MTIX-107.43. The document is hashed as a reader decodes it, so a file whose text held invalid UTF-8 still verifies (MTIX-107.39). `mtix import` MUST recompute and verify this checksum before proceeding; mismatch MUST abort the import with error: `"Export checksum verification failed: file may be corrupt or tampered"`
...
- **Annotations and activity in merge mode (MTIX-95.31.1):** a merge MUST NOT drop a local annotation or activity entry. Annotations merge as a union by annotation id, so an incoming node without annotations keeps the local ones; for an id both sides hold, the local copy wins unless the incoming copy is resolved and the local one is not (a resolution never regresses). Activity entries merge as a union of distinct entries, each identified by its id, type, author, text and time together, so two different entries that share an id are both kept. A file whose major `schema_version` is below 2 carries none of the node fields 2.0.0 added, so a merge MUST keep their local values.
- **Keep what a stale copy leaves empty (MTIX-95.31.4):** when the file's copy of a node is not current by the FR-15.2i rule (the file carries no activity, the copy lacks a local activity entry of the node, or its `updated_at` is older), a merge MUST keep every non-empty local field value the copy leaves empty, the field losses FR-15.2i lists; when one of them describes the node's status (a closed or wake time, an assignee, the invalidation fields), the local status fields MUST be kept together, and when one is content, the content hash MUST be recomputed. A current copy's clear applies. `mtix import --mode merge` MUST take the FR-15.2f pre-import backup once every check has passed and before it writes, and MUST NOT take one when the merge would change nothing.
- **A task another clone renumbered (MTIX-95.31.4):** a local node whose uid the file holds under another id under the same parent is the same task: a merge MUST move it (and its subtree) to that id before it looks for different tasks, without confirmation, and report the move; under another parent it is a uid conflict (ADR-003 §6, F-3).
- **A different task under the same id (MTIX-95.31.4):** when an imported node and the local node that holds its id both carry a uid and the uids differ, they are the same task only when their `created_at` values are equal and at least one uid was assigned after the task was created: a marked backfill uid (below), whatever its time, or a UUIDv7 whose embedded time is more than one hour after `created_at`, a uid a clone of an earlier release assigned by backfill when it upgraded from before uids were shared. Every other pair is two different tasks: a UUIDv7 time before `created_at` (a node a hub applied records the apply time), within the hour after it (a create that waited for the write lock), a uid that is neither a UUIDv7 nor a marked backfill uid, or creation times that differ or are missing. When the file's node has no uid to compare (a board written before uids were shared), the titles decide (MTIX-95.31.9); a local node never lacks one for this rule, because a merge gives it a backfill uid first and the automatic import's comparison treats it the same way, so the refusal, the merge and its adoption report (which shows that local uid as `uid=(new)`) agree: the same title is the same task, merged as before, and another title is a different task, renumbered as below; a renumbered local node without a uid MUST be given a new one, reported as `uid=(new)` and left out of a remap file written without `--confirm`, and the report MUST list such a pair with both titles. Every node an import (replace, merge or the automatic import) inserts without a uid MUST be given a backfill uid inside the import's transaction, and a store MUST give every node it still holds without a uid (NULL or empty) a backfill uid when it opens, logging how many (idempotent). That backfill is a write like any other: it runs through the write transaction, so the NFR-2.8 free-space pre-flight may refuse it (then it is skipped with a log and the open goes on) and a fatal storage error latches fail-stop; a failure never keeps the store from opening, and the next open retries. The pre-v3 migration fills uids only from create events, leaving the rest to this backfill, so a pre-v3 store also opens below the floor, with reads working. With `MTIX_SKIP_INTEGRITY_CHECK=1` (reading a damaged store) the backfill on open writes nothing and logs that it skipped. A merge MUST give every local node still without a uid a backfill uid before it decides any identity, written in its transaction (a merge that is not applied writes nothing). A replace MUST keep, for a file node without a uid, the marked backfill uid of the local node at the same id when their titles match (unless the file holds that uid on another node); a file node with another title is a different task and gets a new backfill uid, so it never inherits the replaced task's uid, so a board without uids never changes such a uid, and FR-15.2i does not count that uid as lost; a create-time uid the file leaves out stays a loss. So every 2.0.0 board mtix writes carries a uid for every node. A backfill uid is a UUIDv8 whose 12 bits after the version hold the fixed marker `0xBF1`: it can never equal a UUIDv7 a create mints, export and import keep it unchanged, and the rule above treats it as a uid assigned after the task was created, whatever its time, so two clones that each backfilled the same task (equal `created_at`) hold one task. The rule is asymmetric on purpose: calling two tasks different costs a renumber the user confirms, calling them the same loses one. A create MUST mint the uid right after it reads the clock, before any step that waits for the write lock. For different tasks, a merge MUST NOT overwrite the local node. For the same task, the local node adopts the file's uid; an empty uid MUST NOT replace a local one. It MUST renumber the local node and its subtree, in the import's transaction, to the next number free under the same parent in both the store and the file (the number after the highest either holds), keeping their uids, and then import the file's node at the id, so the published board keeps its numbers. The move MUST be reported as a uid-keyed remap (uid, old id, new id; `--remap-file` writes it) and applied only with `--confirm` (ADR-003 §6); without it nothing is written. A merge that would still overwrite such a node MUST fail with a conflict and write nothing.
- **Uid adoptions are reported, and the residual case (MTIX-95.31.6):** a merge MUST list every uid it adopts (a local node that takes the file's uid under the same id) with the id, the local uid, the file uid, the local title and the file title: in its report on stderr, applied or not; in `--json` (`uid_adoptions`) once the import is applied (an import that waits for `--confirm` prints no JSON); and in the remap file, where the local uid maps to the id. The identity rule above has one known residual: two different tasks created in the same second, when at least one clone assigned its uid after the task was created (at upgrade, or as a backfill uid on import or open, as it does for a task whose create event its event log lacks, for example one created before 0.2), carry a uid that counts as assigned later, so they are treated as one task and the merge keeps the file's task under the id. The report MUST show both titles and, when they differ, say that the backup taken before the merge holds the local task. Restoring that backup drops every change made after the merge, so the user recovers right away, or notes the later changes and redoes them. To recover it, the user copies that backup over the database and, before any mtix command runs, restores the pulled `.mtix/tasks.json` (the merge's own export rewrote it, and the next command would export the restored store over it); the next command refuses again, the user copies the local task to a new one (writes stay local while the refusal is pending) and merges with `--confirm`. The automatic import MUST list such a pair whose titles differ as `treated as the same task (uid assigned at upgrade)` with both titles, and refuse (FR-15.2i); its merge option MUST say that the merge takes the file's title and content for such a task, and that copying the local task to a new one first keeps it, so the user can refuse the merge. The opposite case is safe but can look wrong: two copies of one task whose uids were all assigned less than an hour after it was created carry uids that count as minted at creation, so they are listed as `a different task under this id` with identical titles, and the merge renumbers the local copy only with `--confirm`.
- **Replace mode writes every field (MTIX-95.31.1):** a replace import MUST write back every exported node field, annotations and activity included, so an export followed by a replace import leaves the store unchanged.
- **Time validation (MTIX-95.31.1):** before writing anything, import MUST reject a time value that is not RFC 3339 or whose UTC year is outside 1..9999 (node timestamps, annotation and activity times, dependency, agent and session times), and an empty value in a required time field (node `created_at` and `updated_at`, dependency `created_at`, session `started_at`), with an error naming the record and the field.
...
sync:
  ...
  team_id: ""
  auto_sync: true              # Automatic import of a changed .mtix/tasks.json (FR-15.2j); false turns it off; exports never overwrite a pulled board (FR-15.3e)
  interval: "30s"
...
**FR-15.1** mtix MUST maintain a git-tracked export file at `.mtix/tasks.json` containing the full task hierarchy in the same format as `mtix export`. "Full" means every column of every node, including annotations (in the structure `mtix show --json` returns) and the activity stream, covered by the export checksum (`schema_version` 2.0.0, MTIX-95.31.1). A nodes column MAY be left out only if it is derived, local-only or a cache, and each one left out MUST be listed with its reason in `docs/EXPORT-FORMAT.md`; today none is. A file written by mtix MUST verify when read back, including a copy restored by git (MTIX-107.39).

**FR-15.2** (Auto-import, amended by MTIX-95.31.2) On startup, unless `sync.auto_sync` is false (FR-15.2j), mtix MUST read `.mtix/tasks.json` exactly once into memory, compute its SHA-256 hash, and compare the hash against a stored hash (in `.mtix/data/sync.sha256`). If the hashes differ — indicating the export file was updated by a `git pull`, `git checkout`, or branch switch — mtix MUST import from the already-read buffer using replace mode within a single database transaction and update the stored hash only after the transaction commits. This eliminates the TOCTOU race between hash check and file read. The import needs no user intervention, but it MUST NOT be silent (FR-15.2k), and it MUST NOT delete local data the file lacks: such an import is refused (FR-15.2i).

**FR-15.2a** (amended by MTIX-95.31.2) If `.mtix/tasks.json` exists but no database exists (fresh clone), mtix MUST initialize the database and import the export file automatically. A local store that holds no nodes MUST be imported whatever `sync.auto_sync` says (FR-15.2j): nothing in it can be lost, and a clone that skipped the import would overwrite the board and reuse its ids on its first write.

**FR-15.2b** If `.mtix/tasks.json` does not exist (new project), auto-import MUST be skipped silently.

**FR-15.2c** Auto-import MUST NOT run for the `init`, `export`, `import`, `help`, or `version` commands to avoid circular dependencies. The commands are matched by their full path below `mtix` (MTIX-95.31.2): the skipped set is `mtix init`, `mtix export`, `mtix import`, `mtix migrate`, `mtix help`, `mtix version` and `mtix sync` without a subcommand, so a subcommand that shares a name (`mtix sync init`, `mtix sync migrate`) auto-imports. A skipped command that writes still auto-exports afterwards, and that export imports a changed board first (FR-15.3e): `mtix import` of another file, for example, is followed by the auto-import of a changed `.mtix/tasks.json` (MTIX-95.31.4).

**FR-15.2d** (Atomic import) Auto-import MUST execute within a single database transaction. If any part of the import fails (schema mismatch, corrupt data, constraint violation), the entire transaction MUST be rolled back, leaving the database unchanged. Partial imports are a data integrity violation. The replace MUST re-check, inside its transaction, that the store's nodes and dependencies are unchanged since the FR-15.2i comparison (the same export checksum); if they changed, it MUST write nothing and leave the stored hash, so the next command runs FR-15.2h and FR-15.2i again (MTIX-95.31.4).
...
**FR-15.2f** (Backup before import, amended by MTIX-95.31.2) Before executing a replace-mode auto-import, mtix MUST run every check the import makes (format, schema version, node count, checksum, time values, a readable local store, FR-15.2h and FR-15.2i) and then create a verified backup snapshot (`VACUUM INTO`, then `PRAGMA quick_check`) at `.mtix/data/backups/pre-sync-<UTC time as YYYYMMDD-HHMMSS>.db`, with a `-2`, `-3`, ... suffix for further imports in the same second. A file that fails a check or is refused MUST NOT cause a backup. Every import that applies MUST take a new backup of the store as it then is, even of a file imported before; only a retry of an import that failed after its backup MAY reuse that backup, and only while the local store is unchanged (keyed by the file's content hash and the hash of the store's export). mtix MUST keep the newest 5 of these pre-import backups and delete older ones only after a new backup succeeds; it MUST NOT delete any other file in that directory. If backup creation fails, auto-import MUST be skipped with a warning. (mtix 0.5.3 and earlier kept one overwritten copy at `.mtix/data/pre-sync-backup.db`; 0.5.4 no longer writes or deletes it.)

**FR-15.2g** (Schema version check) `.mtix/tasks.json` MUST include a `schema_version` field (semver). Auto-import MUST reject files with a major version higher than the running binary supports, logging an error: "tasks.json schema version X.Y.Z is newer than supported version A.B.C — upgrade mtix". `mtix import` MUST apply the same check and refuse such a file with the same message before writing anything (MTIX-95.31.1). Minor/patch version differences MUST be accepted. A format change that an older binary would read with loss MUST raise the major version, so that the older binary skips the file instead: mtix 0.5.4 writes 2.0.0, because a 1.x reader drops the node fields 2.0.0 added (MTIX-95.31.1), and still reads 1.x files.

**FR-15.2h** (Conflict detection) If both the database and `.mtix/tasks.json` have changed since the last sync (i.e., stored hash differs from file hash AND database state differs from stored hash's snapshot), mtix MUST log a warning identifying the conflict and skip auto-import. The user MUST resolve the conflict explicitly via `mtix import --mode replace` or `mtix export`. A successful auto-import MUST refresh the database-state snapshot (`.mtix/data/sync-db.sha256`), so a second changed file with no local write in between is not a conflict (MTIX-95.31.2). When a replace of the file would also lose data (FR-15.2i), the conflict MUST keep that loss list: it is printed as a refusal, recorded, and the conflict's replace option names what it would delete (MTIX-95.31.4).

**FR-15.2i** (Refuse a lossy replace, MTIX-95.31.2) Before a replace-mode auto-import, mtix MUST compare the node and dependency data of the local store with the file, node by node; agents and sessions are runtime state and are replaced without comparison. These are always a loss when the file lacks them: a node, an annotation (by id), an annotation's resolution, an activity entry (by id, type, author, text and time) and a dependency (from, to, type); a dependency a teammate removed on purpose is therefore refused too. A node whose id the file gives to a different task (both carry a uid, the uids differ, and they are not the same task by the FR-7.8 rule: equal `created_at` and a uid minted more than an hour after it, a backfill; or the file gives the id to another local task, moved there) is a loss of the whole local task, named "a different task under this id" with both titles (MTIX-95.31.4). A node the file holds under its id with another title while either has no uid to compare is a loss too, named "a task under this id with a different title and no uid to compare" with both titles, and the merge option says the merge renumbers the local task with `--confirm` (MTIX-95.31.9). A node the file holds under its id with another uid, the same task only by the FR-7.8 upgrade rule, is a loss when the two titles differ: the two may be different tasks that rule cannot tell apart, so it is named "treated as the same task (uid assigned at upgrade)" with both titles, and the merge option names every such node, says the merge takes the file's title and content for it, and says to copy the local task to a new one first when the two are different tasks (MTIX-95.31.6). A local node whose uid the file holds under another id is the same task renumbered by another clone: it is compared with that copy, and a dependency on it is held when the file holds it at the new id; it is not a loss. A non-empty field value the file leaves empty (`node_type`, which import derives from depth, excepted) is a loss unless the file's copy of the node is current: the file carries activity (schema 2.0.0 or later), its copy holds every local activity entry of the node, and its `updated_at` is not older than the local one (an unreadable `updated_at` is never current). Activity is append-only and the writes that record none (update, delete) still move `updated_at`, so a field a current copy cleared was cleared on purpose (unclaim, reopen, undefer, undelete, a later update) and applies. A 1.x file carries no activity and is never current. Clock skew that makes a current copy look older refuses, the safe side. The rule fails open in the other direction: a copy with a later `updated_at` that missed a local update or delete (neither records activity), after a concurrent edit to the same node or with a teammate's clock ahead, counts as current, so a field it leaves empty is applied as cleared; the pre-import backup keeps the state before (MTIX-95.31.4). A stale copy that changes a non-empty value (rather than clearing it) is applied, as a checkout of an older board does on purpose; the pre-import backup keeps the state before. On a loss, mtix MUST refuse the import and change nothing: no import, no backup, and the stored hash is not updated, so the refusal repeats on every command until the user acts. The refusal MUST be printed once per command, MUST name what would be lost, node by node, and MUST name the commands that proceed deliberately, with what each keeps and loses: `mtix import .mtix/tasks.json --mode merge` (backs up the database first and keeps every value the refusal lists: every local node, annotation, activity entry and dependency, and every field value listed, with the node's local status when the value is part of it (FR-7.8); for a node whose content is unchanged the local field values win, so it can undo a teammate's field change; it never overwrites a local task that is a different task than the file's under the same id: it renumbers the local task and its subtree to the next number free under the same parent in both the store and the file, keeping the uid, while the file's task keeps the id, and per ADR-003 §6 reports the uid-keyed remap and applies it only with `--confirm`, MTIX-95.31.4), `mtix sync --fix` (keeps the local store and rewrites `.mtix/tasks.json` from it) and `mtix import .mtix/tasks.json --mode replace` (the file wins). Each of the three resolves the refusal. A file that fails its checks (not an export, unparsable, wrong node count, checksum or times) MUST be recorded as a pending refusal of kind `invalid_file` too. Every recorded refusal has a kind (`lossy`, `conflict`, `newer_schema`, `invalid_file`, `unreadable_store`, `backup_failed`, `not_imported`), and every message that names a way out MUST name the one for that kind; for `newer_schema` it is an upgrade, never `mtix sync --fix`. A file that only adds or changes data is not lossy and is imported. This protects a store from a board re-exported by a client older than 0.5.4 (`schema_version` 1.0.0, without annotations or activity) and from a teammate's stale board.

**FR-15.2j** (Auto-import switch, MTIX-95.31.2) The `sync.auto_sync` config key (default `true`) MUST switch auto-import off when it is `false`, except that a local store holding no nodes is always imported (FR-15.2a). `.mtix/config.yaml` is tracked in git, so the switch applies to every clone. `mtix config set sync.auto_sync` MUST accept only true or false (as `strconv.ParseBool` reads them) and reject any other value, changing nothing. A value in the file that is neither (written by hand) MUST leave auto-import on, the default, with a one-line warning. The switch MUST NOT switch off auto-export (FR-15.3), but auto-export never overwrites a pulled board (FR-15.3e): with the switch off, a changed `.mtix/tasks.json` stays until it is imported with an explicit `mtix import` or replaced with `mtix sync --fix`.

**FR-15.2k** (Auto-import notice and state, MTIX-95.31.2) Every auto-import that applies a changed `.mtix/tasks.json` MUST print one line on stderr saying how many nodes it added, updated and removed, how many dependencies it added and removed, and where the pre-import backup is. `mtix sync` MUST report the configured `sync.auto_sync` value, whether auto-import is enabled, and the last auto-import mtix refused or skipped for the user's decision (FR-15.2f, FR-15.2g, FR-15.2h, FR-15.2i, or a local store that cannot be exported), with its time, its reason and whether it is pending or resolved, in text and in `--json` output (`auto_import`). `mtix sync` MUST NOT report the store and the file as in sync while an auto-import of the file is pending (the headline says so) or while an id is held under different uids (listed, `different_uid` in `--json`) (MTIX-95.31.4).
...
**FR-15.3e** (Never overwrite a pulled board, MTIX-95.31.2) Auto-export, from every writer (CLI commands, `mtix mcp`, `mtix serve`, the daemon), MUST NOT overwrite a `.mtix/tasks.json` that changed on disk since mtix last wrote or imported it (its hash is neither the stored hash nor the database's last export hash). It MUST run the auto-import path first: if that imports the file, the export proceeds; otherwise the file is kept, the refusal is recorded as pending (FR-15.2i; kind `not_imported` when nothing else was recorded), the write stays in the local store, and one line per command says so with the way out for that kind. While a refusal is pending for the file on disk, auto-export MUST keep it the same way. `mtix sync --fix` rewrites the file from the store without importing it, and an explicit `mtix import` of the file on disk records it as imported; both resolve the refusal.
...
### FR-18: Local-First Team Sync (BYO Postgres Hub)

> mtix v0.1.x is a single-developer tool: the local SQLite is the system of record, and `tasks.json` plus git is the only sharing mechanism. v0.2 adds a local-first sync model: each developer keeps their own SQLite (so offline work and instant reads remain unchanged), and a shared Postgres hub stores an ordered event log that every CLI pushes to and pulls from. The complete protocol design lives in [docs/SYNC-DESIGN.md](docs/SYNC-DESIGN.md); this section is the normative requirements that bind the implementation.

**FR-18.1** (Opt-in adoption) Sync MUST be opt-in. A user that never runs `mtix sync init` or `mtix sync clone` MUST see no behavior change from v0.1.x. All existing v0.1.x tests MUST continue to pass with no modification.

**FR-18.2** (Local-first reads) Every CLI command that does not push or pull MUST read exclusively from the local SQLite. Sync code paths MUST NOT block command execution on hub reachability; commands operate on local state.

**FR-18.3** (Atomic event recording) Every mutation that writes to the `nodes` table MUST also write a `sync_events` row in the same SQLite transaction. A chaos test MUST prove that SIGKILL between the node write and the event write leaves the database in a state where either both rows persist or neither row persists — never one without the other.

**FR-18.4** (Hub schema) The hub MUST provision the following tables on first connect: `sync_events`, `sync_conflicts`, `sync_projects`, `applied_events`, and `audit_log`. Schema migration MUST single-flight via PG advisory lock (`pg_advisory_xact_lock`) so that N concurrent first-connecting CLIs result in exactly one migration runner. A partial-migration test (SIGKILL between two ALTER TABLE statements) MUST prove the next CLI re-runs the full migration cleanly with no half-applied schema.

**FR-18.5** (Append-only audit) PG triggers MUST raise an exception on `UPDATE` or `DELETE` against the `audit_log` and `sync_conflicts` tables. The trigger MUST NOT be removable by the application role used by mtix (least-privilege role MUST NOT have permission to alter or disable triggers).

**FR-18.6** (Event format) Every sync event MUST contain the fields enumerated in SYNC-DESIGN §3.1: `event_id` (UUID v7), `project_prefix`, `node_id`, `op_type` (one of 12 enum values), `payload` (JSONB), `wall_clock_ts` (int64 ms), `lamport_clock` (int64), `vector_clock` (JSONB map), `author_id`, `author_machine_hash`, `created_at`. The local mirror additionally includes `sync_status` (`pending` | `pushed` | `conflicted` | `applied`).

**FR-18.7** (Hub-side validation) The hub MUST reject any event that fails the validator rules in SYNC-DESIGN §5.1 with structured error code `MTIX_SYNC_EVENT_INVALID` plus the failing field name. The entire `PushEvents` batch MUST fail atomically if any event in it is invalid; partial-batch acceptance is forbidden. Validator constraints include payload size `<= 64 KB`, JSON nesting depth `<= 10`, vector-clock map `<= 100 entries`, each VC entry `< 2^53`, `author_id` matches `^[a-z0-9_-]{1,64}$`, `op_type` is one of the 12 documented enum values.

**FR-18.8** (Timestamp validation) The hub MUST reject events with `wall_clock_ts > now + 24h` with structured error `SYNC_TIMESTAMP_FUTURE`. The hub MUST accept (with a WARN log) events with `wall_clock_ts < now - 30d` to permit replay from long-offline laptops. Conflict resolution MUST use `lamport_clock` as the primary ordering input; `wall_clock_ts` is the first tie-break only.

**FR-18.9** (Idempotent apply) The local apply engine MUST treat a duplicate `event_id` (already in `applied_events`) as a no-op (return success, log at debug). A property-based test MUST generate at least 1000 random sequences of 100 events across 10 nodes; for each sequence, applying in any causal-respecting order MUST produce byte-identical local state (verified by content_hash). A chaos test MUST send SIGKILL at parameterized 5ms intervals across the apply transaction and verify the event is either fully applied (and recorded in `applied_events`) or fully unapplied (and absent from `applied_events`) — never partial.

**FR-18.10** (NodeTypeForDepth canonicalization carry-over) On every event apply that touches `node_type`, the apply engine MUST ignore any non-canonical value in the event payload and use `NodeTypeForDepth(node.depth)`. This carries forward the FR-7 / MTIX-12 contract: a peer CLI with a buggy local mtix MUST NOT be able to poison the hub's hierarchy with non-canonical node types. A regression test MUST construct an event with `depth=0` and `node_type='story'` and assert the applied row has `node_type='epic'`.

**FR-18.11** (Conflict resolution) Conflict detection MUST use the vector-clock comparison defined in SYNC-DESIGN §8.1. Resolution for two concurrent same-field events MUST be deterministic per SYNC-DESIGN §8.2: higher `lamport_clock` wins; tie-break by higher `wall_clock_ts`; final tie-break by lower `author_machine_hash` (lexicographic). No randomness MUST appear in the resolution path. Delete-vs-update MUST always favor the tombstone (deletes are monotonic).

**FR-18.12** (Conflict surfacing) Every non-trivial resolution MUST log a row to `sync_conflicts` (hub) and a line to `.mtix/conflicts.log` (local). When a node has unresolved conflicts, `mtix context <node_id>` MUST append a `CONFLICT` block listing each conflicting field with both candidate values. AGENTS.md MUST instruct agents to escalate via `mtix_comment` rather than silently choose. When unresolved conflicts exceed 50 for one project, `mtix sync status` MUST surface a banner with batch-resolution guidance.

**FR-18.13** (Divergent-history detection) On `mtix sync clone` or first `mtix sync push` after `mtix sync init`, the CLI MUST query `sync_projects` for the local `project_prefix` and compare `first_event_hash`. On mismatch the CLI MUST refuse with structured error `MTIX_SYNC_DIVERGENT_HISTORY` and offer four resolution paths: `--discard-local`, `--rename-to NEWPREFIX`, `--import-as PARENT-ID`, `--dry-run`. `--rename-to` MUST first check the hub for a project with the new prefix and refuse with `MTIX_RECONCILE_PREFIX_COLLISION` before any local mutation. A reconciliation atomicity test MUST inject synthetic failures per resolution path and prove either a clean rollback or a recoverable resume marker.

**FR-18.14** (Protocol versioning) The constant `MTIX_SYNC_PROTOCOL_VERSION` MUST be defined and start at 1. Every `PushEvents` and `PullEvents` request MUST include the client protocol version in a metadata envelope. The hub MUST refuse cross-major requests with structured error `SYNC_VERSION_MISMATCH` plus a one-line upgrade hint. Minor version bumps MUST be additive only; old clients MUST tolerate unknown optional fields. New `op_type` values MUST require a major version bump. The hub MUST record the highest seen `client_version` per project as `last_seen_cli_version` for advisory drift detection by `mtix sync doctor`.

**FR-18.15** (TLS posture) The PG transport MUST default to `sslmode=verify-full`. Weaker modes (`verify-ca`, `prefer`, `disable`) MUST be refused unless `--insecure-tls` is set AND the host resolves to loopback. The transport MUST honor the `MTIX_SYNC_SSLROOTCERT` environment variable and the DSN `sslrootcert=` parameter for managed PG providers that require a custom CA bundle. A test using a self-signed cert chain MUST prove the SSLROOTCERT path works.

**FR-18.16** (DSN sourcing) The hub DSN MUST be sourced from exactly one of: the `MTIX_SYNC_DSN` environment variable, or the `.mtix/secrets` file (mode 0600, gitignored). DSN sourcing MUST refuse to load from any tracked YAML/JSON config file with structured error `MTIX_SYNC_DSN_IN_TRACKED_FILE`. CLI flag `--dsn` MUST be refused (process listings expose flags). A regression sweep MUST place a DSN in every plausible misconfiguration (tracked config, `.mtix/secrets` mode 0644, CLI flag) and verify each fails closed before any network call.

**FR-18.17** (DSN never in observable output) Every log message, error message, panic message, and MCP tool output that crosses a process boundary MUST pass through `RedactDSN` (extending the redactor introduced in MTIX-14.9). Top-level `main()` and every spawned goroutine MUST install a `defer-recover` that runs the recovered value through `RedactDSN` before printing. A test MUST deliberately panic with a real DSN in scope and verify the printed panic does not contain the DSN. A regression sweep MUST inject a synthetic DSN string and grep all stdout, stderr, log files, and MCP tool outputs for any leak.

**FR-18.18** (Singleton pusher) A background event pusher MUST acquire `.mtix/sync.push.lock` via `flock(LOCK_EX | LOCK_NB)`. When N concurrent agents on one machine emit mutations, exactly one process MUST become the active pusher; others MUST write events to the local queue and exit immediately. The lock MUST be auto-released on process exit (kernel-level guarantee; no manual cleanup required). A chaos test with 10 concurrent processes MUST verify exactly one pusher.

**FR-18.19** (Hook-friendly transient errors) When the environment variable `MTIX_SYNC_HOOK=1` is set, every sync command MUST exit 0 with a `WARN` line on stderr on transient PG errors (network, statement_timeout, auth retry). This is the mode used by git pre-push hooks; the hook MUST never block a code push because the sync hub is degraded. Hard errors (DSN missing, tracked-file DSN, malformed event) still fail-closed.

**FR-18.20** (Streamed clone) `mtix sync clone` for projects above 10,000 events MUST pull in batches of 1000 events with a progress bar. After every batch, the CLI MUST write `meta.sync.clone.checkpoint` so that `mtix sync clone --resume` recovers from interruption without restarting from event zero.

**FR-18.21** (Backup primitive) `mtix sync backup --output FILE` MUST invoke `pg_dump` on the mtix-owned tables (`sync_events`, `sync_conflicts`, `sync_projects`, `applied_events`, `audit_log`) and write a portable SQL dump. A round-trip test MUST: backup a populated hub, drop the hub schema, restore from the dump, and verify the event count and `first_event_hash` for every project match the pre-backup state. The corresponding restore command (`psql < FILE`) MUST be documented in workflows/safety-critical.md.

**FR-18.22** (Reconciliation audit trail) Every reconciliation MUST emit structured audit events: `RECONCILE_START` (path, counts), `RENAME_NODE` (old_id, new_id) per node, `RECONCILE_DONE` (final stats). A `.mtix/id-rename-map.json` file MUST be persisted on success so post-reconciliation agent sessions can look up renamed IDs. On failure the file MUST be absent or marked `partial=true`.

**FR-18.23** (Privacy / PII statement) mtix MUST NOT classify or scrub PII from any node text field; the hub stores user-supplied content verbatim. mtix is NOT GDPR/HIPAA/PCI-compliant. The user manual and SECURITY-MODEL.md MUST state this explicitly. Teams requiring true right-to-erasure MUST be directed to either the planned hosted SaaS or a full hub-database rotation. `mtix delete` writes a tombstone, NOT a true erasure of the original content from the event log.

**FR-18.24** (Side-channel non-applicability) mtix is NOT a confidentiality boundary inside a team. Timing-attack mitigations and cache-side-channel mitigations are explicitly out of scope. SECURITY-MODEL.md and SYNC-DESIGN.md MUST state this so future contributors do not waste effort hardening against a non-threat. If mtix ever ships a multi-tenant mode, this exclusion MUST be revisited and the relevant documents bumped.

**FR-18.25** (Operating envelope) The supported operating envelope MUST be documented in SYNC-DESIGN.md §6.2: <=10 active developers per hub, <=10K nodes per project, <=1M events per project lifetime, <=10 concurrent agents per laptop. Soft warnings and hard ceilings MUST be defined for each. Beyond the supported envelope, mtix MAY degrade gracefully but MUST NOT silently corrupt state.

**FR-18.26** (Compaction reservation) The v1 schema MUST include `sync_events.retained_until TIMESTAMPTZ NULL` and the setting key `hub.events_retention_days` (default `0` = forever). Both MUST be no-ops in v1 with explicit TODO comments referencing v2 compaction. The schema reservation makes the v2 transition non-breaking.

**FR-18.27** (Fuzz testing) The transport and validator code MUST ship Go native fuzz targets covering at least `FuzzEventDecode` (random bytes into the event JSON decoder; never panic), `FuzzVectorClockMerge` (commutativity property: `merge(a, b) == merge(b, a)`), and `FuzzPushEventsValidation` (random Event structs; the validator MUST either accept or reject with a structured error — never panic, never partially apply). CI MUST run fuzz targets for at least 10 minutes on a dedicated job.

**FR-18.28** (Audit pass 2) MTIX-15.11 MUST re-verify every audit item from the MTIX-15.1 design audit against the shipped code. The sign-off comment on MTIX-15.11 MUST list, for each audit item, the test file path and test name that proves it.
...
**NFR-2.6a** Open-time integrity check: BEFORE the first connection is opened, the store MUST validate the database file header — a header page count exceeding the file's actual size (the torn-checkpoint signature) MUST be refused unless a non-empty WAL exists for SQLite to replay, because opening such a file can reset the WAL and destroy the evidence a recovery needs. After opening the write connection and before any write, the store MUST execute `PRAGMA quick_check` and refuse all operation on failure with recovery guidance (restore from backup or the tasks.json mirror; see USERMANUAL "Disk full and corruption recovery"). This applies to EVERY process that opens the store — CLI, MCP, serve — not just `mtix serve`; quick_check cost at mtix scale is milliseconds. The `MTIX_SKIP_INTEGRITY_CHECK=1` environment variable bypasses BOTH open-time gates (header validation and quick_check) with a loud error-level log line — recovery commands (`verify`, `backup`, `export`) MUST be able to reach a damaged database, or the documented recovery procedure is a dead end. The bypass MUST never be used to keep writing.
...
**NFR-2.8** Disk-full safety (added 2026-06 after a field incident in which a user's database was torn by a checkpoint on a 99%-full volume and the data became unrecoverable):

- **Explicit durability pragmas.** Write connections MUST set `synchronous=FULL` and `wal_autocheckpoint=1000` explicitly (NFR-2.1). Driver defaults MUST NOT be relied on.
- **Free-space pre-flight.** Every write transaction MUST be preceded by a free-space check on the volume holding the database; below the floor (default 8 MiB, override `MTIX_MIN_FREE_BYTES`, `0` disables) the write MUST be refused with an actionable error naming the volume, the free bytes, and the override. `mtix backup` MUST additionally require the destination volume to hold the database plus WAL plus the floor before starting `VACUUM INTO`. Reads MUST keep working while writes are refused.
- **Fail-stop on fatal I/O errors.** Any write-path error indicating filesystem failure (SQLITE_FULL, SQLITE_IOERR, SQLITE_CORRUPT, ENOSPC, EIO) MUST latch the store into fail-stop: every subsequent write in that process is refused with the original cause. Continuing to write into a failing filesystem is never permitted. A fail-stopped store MAY still serve reads and back up to a different volume. The latch guards the integrity of the local store only: a failure on any other medium — a sync transport's directory, an export destination, a backup volume — MUST NOT trip it, because a transport that cannot be written is degraded delivery, not a damaged database.
- **Open failures must name disk pressure.** When opening the database fails and the volume is below the floor (or nearly empty), the error MUST say so — `SQLITE_CANTOPEN` alone sends users hunting for corruption that isn't there.
- **Mirror parity across interfaces.** The FR-15.3 tasks.json auto-export MUST fire for mutations made through EVERY interface, including long-running ones (MCP server, `mtix serve`), via the store's on-commit hook with debouncing — not only via CLI process exit. A failed export MUST be loud (error log) and MUST never destroy the previous good mirror (atomic temp+rename).
- **Automated rolling backups (MTIX-26.6).** After mutations, if the newest backup under `{data.dir}/backups/` is older than the interval (default 24h, override `MTIX_BACKUP_INTERVAL`, `0` disables), mtix MUST take a verified backup (`VACUUM INTO` + quick_check) and rotate to the newest N (default 7, override `MTIX_BACKUP_RETAIN`). The trigger rides the same post-mutation paths as the mirror (CLI PostRun; debounced on-commit hook for MCP/serve/daemon). A failed automatic backup MUST be logged, MUST NOT fail the triggering command, and MUST NOT disturb existing backups.
- **Recovery tooling (MTIX-26.5).** `mtix recover` MUST salvage readable rows from a damaged database (read-only, per-row reads, `cell_size_check=OFF`), merge unreadable rows from the tasks.json mirror, synthesize placeholder parents to keep the FK chain importable, and emit an importable export plus a salvage report — without modifying the damaged files. `mtix import --recompute-checksum` MUST (loudly) accept hand-reconstructed exports.
- **Conformance evidence.** The fault-injection suite (`e2e/faultinject`, build tag `faultinject`) MUST pass on every CI build. It drives the real binary on a deliberately tiny volume through disk-full writes, genuine ENOSPC, kill -9 mid-write, and the recorded incident signature (in-header page count exceeding file size), asserting the database is always either intact or cleanly refused — never silently torn.
