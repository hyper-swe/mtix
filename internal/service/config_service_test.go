...
// seven sync.relay.* keys; the number is asserted so a key can never be
// added without the FR-11.2 allowlist being updated deliberately.
func TestConfig_ValidConfigKeys_Returns36Keys(t *testing.T) {
	keys := service.ValidConfigKeys()

	assert.Len(t, keys, 36, "should return exactly 36 valid config keys")
	...
	assert.Contains(t, keys, "author_id")
	assert.Contains(t, keys, "mcp.read_only")
