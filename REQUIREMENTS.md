...
- **Uid adoptions are reported, and the residual case (MTIX-95.31.6):** a merge MUST list every uid it adopts (a local node that takes the file's uid under the same id) with the id, the local uid, the file uid, the local title and the file title: in its report, applied or not, in `--json` (`uid_adoptions`), and in the remap file, where the local uid maps to the id. The identity rule above has one known residual: two different tasks created in the same second on clones running releases before 0.4, when both clones upgraded more than one hour later, both carry uids assigned at upgrade, so they are treated as one task and the merge keeps the file's task under the id. The report MUST show both titles and, when they differ, say that the backup taken before the merge holds the local task; the user recovers it from that backup. The automatic import MUST list such a pair whose titles differ as `treated as the same task (uid assigned at upgrade)` with both titles, and refuse (FR-15.2i), so the user can refuse the merge. The opposite case is safe but can look wrong: two copies of one task, created less than an hour before both clones upgraded, carry uids minted within the hour, so they are listed as `a different task under this id` with identical titles, and the merge renumbers the local copy only with `--confirm`.
