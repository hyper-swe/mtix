...
12. **Restore-Collision Option B (ADR-003 §6.1/§15):** A hub restored from an old backup that re-grants a settled number to a different node is detected as a cross-epoch settled-vs-settled collision; the affected node is blocked (scoped — every other node keeps syncing, F-1), surfaced for admin resolution, and resolved with no node ever deleted. The operator-gated restore-epoch is the un-forgeable discriminator
13. **Restore Near-Miss (no false Option B):** Same-epoch concurrent races, repeats within a restore window, two offline provisionals, and never-restored hubs all take the ordinary renumber path and never queue a restore collision — the regression guard against the rejected uid-age trigger
14. **Atomic Subtree Renumber (ADR-003 §5/F-2):** Renumbering a node rewrites its entire subtree in one transaction; a concurrent reader sees all-old or all-new (never torn); stable uids and references survive; no sync events are emitted
15. **Import-Boundary UID Validation (ADR-003 §6/F-3):** Import validates incoming uid uniqueness against the local store — idempotent re-import is a no-op, a uid colliding with a different local node is rejected (or re-stamped under `--force-rename`), and a crafted export with an internal duplicate uid is rejected, never blindly linked; a live-store apply requires explicit confirmation
