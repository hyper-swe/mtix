...
### Fixed
- **After `mtix sync clone` or `mtix sync pull`, `mtix create` could fail with "already exists" (MTIX-95.38).** Applying a cloned or pulled task did not advance the local ID counter of its parent, so the next `mtix create` under that parent took a number a pulled task already held and failed with `node <ID> already exists`. Each failed create moved the counter on by one, so after a clone of k tasks, or a pull of k new tasks numbered above the counter, k creates failed before one succeeded. A renumber (after the hub rejected a task's number, or when a provisional ID settles) and `mtix sync reconcile --rename-to` and `--import-as` did not update the counters either, so the first create after them could take a number a moved task held. Applying a cloned or pulled task now advances its parent's counter in the same transaction (a re-applied event leaves it unchanged), and a renumber and both reconcile commands raise the counter of every namespace they write ids into; none of these paths lowers a counter (an import still resets every counter to the highest number its tasks hold, as before, which can lower one). A create whose counter is behind for any other reason, for example an import that stopped before it rebuilt the counters, takes the number after the highest one the IDs under the parent hold instead of failing. Task numbers are limited to 2147483647: a pulled task numbered above it is applied without advancing the counter (the apply logs a warning naming it), and a create under a parent whose numbers reach the limit fails with an error that names the limit. Before, a pulled task numbered 9223372036854775807 made every later create under its parent fail with an unclear overflow error.
