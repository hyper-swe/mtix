...
**Related rules of the same change:** a local node that holds an id the file gives to a *different* task (another `uid`) is never overwritten: the merge renumbers the local node to the next number free in both the store and the file, and applies that only **with** confirmation, as §6 requires. Two nodes under one id with different `uid`s are the same task only when their `created_at` values are equal and at least one `uid` is a UUIDv7 whose embedded time is **more than one hour after** `created_at`: a `uid` assigned by the local-mint backfill of §7 when a clone upgraded from before uids were shared. Every other pair is two different tasks: a `uid` time before `created_at` (a node a hub applied records the apply time as `created_at`), within the hour after it (a create that waited for the write lock), or a `uid` that is not a UUIDv7. The rule is deliberately asymmetric: a wrong "different" costs a renumber the user confirms, a wrong "same" loses a task. A create mints its `uid` right after it reads the clock, before any step that waits for the write lock.
