...
**Uid adoptions (merge).** When the file holds a task's id under another uid and one of the two uids was assigned when a clone upgraded (more than an hour after the task was created), `mtix import --mode merge` treats them as one task and gives the local task the file's uid. It lists every adoption, applied or not, under `uids adopted from the file`: the id, both uids and both titles (`uid_adoptions` in `--json`; `--remap-file` maps the local uid to the id). Check every adoption whose titles differ: two different tasks created in the same second on clones running releases before 0.4, both upgraded more than an hour later, are treated as one, the merge keeps the file's task, and the local one survives only in the backup the merge took first. The automatic import refuses such a pair and lists it as `treated as the same task (uid assigned at upgrade)` with both titles. If the titles name two different tasks, do not merge: show both to the human, copy the local task to a new one (`mtix show <id>`, then `mtix create` with its title and description), then merge. If a merge already took it, the human restores the backup (every mtix process stopped, the backup copied over `.mtix/data/mtix.db`) and you copy the task as above before merging again. The opposite case: two copies with identical titles listed as `a different task under this id` (both clones upgraded within an hour of the task's creation) are one task; the merge renumbers the local copy only with `--confirm`, and the human decides whether to delete the duplicate.
