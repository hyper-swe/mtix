...
- **Annotations and activity in merge mode (MTIX-95.31.1):** a merge MUST NOT drop a local annotation or activity entry. Annotations merge as a union by annotation id, so an incoming node without annotations keeps the local ones; for an id both sides hold, the local copy wins unless the incoming copy is resolved and the local one is not (a resolution never regresses). Activity entries merge as a union. A file whose major `schema_version` is below 2 carries none of the node fields 2.0.0 added, so a merge MUST keep their local values.
