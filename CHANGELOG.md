...
### Upgrading
- Upgrading fixes how a **new** backfill stream is generated. It does not repair a stream that already exists, locally or on the hub — if you backfilled on 0.5.1-beta or earlier and any node had been reparented, that stream must be regenerated and re-pushed before a clone will apply.
- **Do not use `mtix sync backfill --force` for this.** `--force` does not regenerate: it appends a second history alongside the first with fresh event IDs, so the badly-ordered events survive (the clone still fails) and the hub, which dedupes on `event_id`, accepts the duplicates. `mtix sync reconcile --discard-local` is also the wrong tool — it drops the local `nodes` table and adopts hub state.
- Until a supported reset lands (MTIX-89), regenerating means clearing the local journal by hand: back up with `mtix backup <path>`, run `DELETE FROM sync_events;` against `.mtix/data/mtix.db`, then `mtix sync backfill` and push. If the bad stream already reached the hub, its rows must also be removed there before the re-push, or both orderings will be present and the clone will still fail.
