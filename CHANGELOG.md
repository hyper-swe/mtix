...
- **The first automatic import after upgrading from 0.3.0 through 0.5.3 refused a teammate's `.mtix/tasks.json` as a conflict although nothing had changed (MTIX-95.31.11).** To tell whether the local store changed since the last sync, the automatic import compares it with the hash recorded at the last automatic export or applied import, `.mtix/data/sync-db.sha256` (FR-15.2h). 0.5.3 recorded the hash of its 1.0.0 export, 0.3.0 the same export without uids (the upgrade gives the tasks their uids), and this release hashes the 2.0.0 export, which adds annotations, the activity stream and the other node fields (MTIX-95.31.1). So every store upgraded from those versions looked changed until a write on this release recorded a new hash. The first board pulled after the upgrade was then refused as "both the local store and the file changed", and a board that would delete local data was reported as a conflict rather than as a loss. The refusal repeated on every command until it was resolved by hand, and none of the ways out it names applies the teammate's board with the loss check: `mtix sync --fix` drops the teammate's changes, the replace import deletes local data the file lacks, and the merge decides per task, keeping the local field values where the task's content matches the file's (so it reverts a teammate's claim) and taking the file's copy where it differs (so it undoes a local edit). The same happened when the hash was recorded while tasks had no uid and a later open gave them backfill uids (MTIX-95.31.9). When the hashes differ, mtix now also hashes the store in those older forms: the 1.0.0 export exactly as 0.5.3 wrote it, the export without its backfill uids, both, and the 1.0.0 export without any uid, as 0.3.0 wrote it. When one of them matches the recorded hash, mtix records the hash of the current form instead, atomically, logs `sync_baseline_upgraded` once, and no longer reports the pull as a conflict, whether or not other commands ran before it. The pull can still be refused as a loss when the local store holds data that boards written by an older version never carried (comments, activity entries, a previous status): a teammate whose store was filled from such boards lacks it, and so does their board. The way out that keeps it today is `mtix import .mtix/tasks.json --mode merge`, which can undo a teammate's change or your own, task by task; afterwards check `git log -p .mtix/tasks.json` and reapply what it undid. A fix is tracked. A local change made while a hash in an older form is recorded is still reported as a conflict, whether it was made before the upgrade and never exported or on this release while a pulled board is kept. The exception is a change only to a field the older form lacks (an annotation, an activity entry or another 2.0.0 field, or a uid), which the older version did not detect either; the loss check still refuses a board that would delete a local annotation or activity entry. One case remains: when the older version's last automatic import was not followed by a writing command, its hash predates that import (read-only commands record none), and the first pull after the upgrade is still reported as a conflict. The way out, in the user manual, applies the pulled board through the automatic import with its loss check: `mtix sync --fix`, then `git checkout HEAD -- .mtix/tasks.json`, then any command that imports, such as `mtix list`. The warning of a conflict and the line a write prints while it is pending no longer say the merge combines both sides: they say, as the `not_imported` line now does too, that per task the merge keeps your field values when the task's content matches the file's and otherwise takes the file's copy, undoing your edits to that task, and the conflict lines name that way out for a conflict when nothing changed locally.
