...
- **The first automatic import after upgrading from 0.5.3 refused a teammate's `.mtix/tasks.json` as a conflict although nothing had changed (MTIX-95.31.11).** To tell whether the local store changed since the last sync, the automatic import compares it with the hash recorded at the last export or import, `.mtix/data/sync-db.sha256` (FR-15.2h). 0.5.3 recorded the hash of its 1.0.0 export, and this release hashes the 2.0.0 export, which adds annotations, the activity stream and the other node fields (MTIX-95.31.1), so every store upgraded from 0.5.3 looked changed until a write on this release recorded a new hash. The first board pulled after the upgrade was then refused as "both the local store and the file changed", and a board that would delete local data was reported as a conflict rather than as a loss. The refusal repeated on every command until it was resolved by hand, and of the ways out it names, only the merge import keeps both sides: `mtix sync --fix` drops the teammate's changes. The same happened when the hash was recorded while tasks had no uid and a later open gave them backfill uids (MTIX-95.31.9). When the hashes differ, mtix now also hashes the store in those older forms: the 1.0.0 export exactly as 0.5.3 wrote it, the export without its backfill uids, and both. When one of them matches the recorded hash, mtix records the hash of the current form instead, logs `sync_baseline_upgraded` once, and imports the board as usual, whether or not other commands ran before the pull. A local change that was never exported is still reported as a conflict. The exception is a change only to a field the 1.0.0 export lacked, which 0.5.3 did not detect either; the loss check still compares it.
