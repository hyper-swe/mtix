...
func TestRunList_TypeFlag_FiltersResults(t *testing.T) {
	...
	err = runList("", "", "", string(model.NodeTypeEpic), "", "", "", 0, false, 50, "", false)
...
func TestRunList_CommaSeparatedUnder_AcceptsMultipleSubtrees(t *testing.T) {
	...
	err := runList("", "TEST-1,TEST-3", "", "", "", "", "", 0, false, 50, "", false)
...
func TestRunList_CommaSeparatedStatus_ParsesCorrectly(t *testing.T) {
	...
	err := runList("done,cancelled", "", "", "", "", "", "", 0, false, 50, "", false)
...
func TestRunList_CommaSeparatedPriority_ParsesCorrectly(t *testing.T) {
	...
	err := runList("", "", "", "", "1,2", "", "", 0, false, 50, "", false)
...
func TestRunList_InvalidPriority_ReturnsError(t *testing.T) {
	initTestApp(t)
	err := runList("", "", "", "", "foo", "", "", 0, false, 50, "", false)
	require.Error(t, err)
...
func TestRunSearch_CommaSeparatedAllFilters_ParsesCorrectly(t *testing.T) {
	...
	err := runSearch("open,in_progress", "agent-1,agent-2", "epic,story", "TEST-1,TEST-2", "", "", 50, "", false)
...
func TestRunList_FieldsFlag_ProjectsJSONOutput(t *testing.T) {
	...
	err := runList("", "", "", "", "", "id,title,status", "", 0, false, 50, "", false)
...
func TestRunList_FieldsFlag_InvalidField_ReturnsError(t *testing.T) {
	...
	err := runList("", "", "", "", "", "id,nonexistent", "", 0, false, 50, "", false)
	require.Error(t, err)
	assert.ErrorIs(t, err, model.ErrInvalidInput)
}
...
func TestRunList_FieldsFlag_NoJSON_Ignored(t *testing.T) {
	...
	err := runList("", "", "", "", "", "id,title", "", 0, false, 50, "", false)
...
func TestRunSearch_FieldsFlag_ProjectsJSONOutput(t *testing.T) {
	...
	err := runSearch("", "", "", "", "", "id,status", 50, "", false)
	assert.NoError(t, err)
}

// --- Phase D: Briefing format tests ---

// TestRunList_BriefingFormat_ProducesOutput verifies that --format briefing
// renders output in the briefing format per FR-17.4.
...
// TestRunList_BriefingFormat_WithFields verifies that --format briefing
// combined with --fields restricts the rendered fields.
...
// TestRunList_BriefingFormat_WithMaxFieldChars verifies truncation.
func TestRunList_BriefingFormat_WithMaxFieldChars(t *testing.T) {
	initTestApp(t)

	require.NoError(t, runCreate("Truncation Test", "", "", 1, "", "", "", "", ""))

	err := runList("", "", "", "", "", "", "briefing", 10, false, 50, "", false)
	assert.NoError(t, err, "--format briefing with --max-field-chars must work")
}

// TestRunList_BriefingFormat_WithShowEmpty verifies that --show-empty
// includes empty fields.
func TestRunList_BriefingFormat_WithShowEmpty(t *testing.T) {
	initTestApp(t)

	require.NoError(t, runCreate("Empty Fields", "", "", 1, "", "", "", "", ""))

	err := runList("", "", "", "", "", "", "briefing", 0, true, 50, "", false)
	assert.NoError(t, err, "--format briefing with --show-empty must work")
}

// TestRunList_BriefingFormat_InvalidField verifies error on bad field.
func TestRunList_BriefingFormat_InvalidField(t *testing.T) {
	initTestApp(t)

	require.NoError(t, runCreate("Node", "", "", 3, "", "", "", "", ""))

	err := runList("", "", "", "", "", "id,bad_field", "briefing", 0, false, 50, "", false)
	require.Error(t, err)
	assert.ErrorIs(t, err, model.ErrInvalidInput)
}
