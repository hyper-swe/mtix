...
- **Switch.** `sync.auto_sync` (default `true`) switches only the automatic import; `mtix config set sync.auto_sync false` turns it off, and `mtix config set` accepts only true or false. `.mtix/config.yaml` is tracked in git, so the switch applies to every clone, but a store with no tasks (a fresh clone) is always imported. With it off, a pulled board is not overwritten by writes; import it with `mtix import .mtix/tasks.json --mode merge` (your status, assignee and wake time win for tasks whose content the teammate did not change: check `git log -p .mtix/tasks.json` and reapply such changes), or keep your board with `mtix sync --fix`.
