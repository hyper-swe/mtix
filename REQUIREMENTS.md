...
- **Fail-stop on fatal I/O errors.** Any write-path error indicating filesystem failure (SQLITE_FULL, SQLITE_IOERR, SQLITE_CORRUPT, ENOSPC, EIO) MUST latch the store into fail-stop: every subsequent write in that process is refused with the original cause. Continuing to write into a failing filesystem is never permitted. A fail-stopped store MAY still serve reads and back up to a different volume.
