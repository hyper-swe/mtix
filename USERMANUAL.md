...
| `conflict` | Both the file and the local store changed since the last sync | Merge them with `mtix import .mtix/tasks.json --mode merge` (for a task whose content the teammate did not change, your status, assignee and wake time win over theirs: afterwards check `git log -p .mtix/tasks.json` and reapply such changes), keep the local store with `mtix sync --fix`, or keep the file with `--mode replace`. If you changed nothing locally, use the way out under "Conflict baseline" above instead. When the replace would also delete local data, the conflict is printed as a refusal that lists it, and the line names what the replace deletes |
